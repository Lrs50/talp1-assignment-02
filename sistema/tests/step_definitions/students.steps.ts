...
import { getStudents, saveStudents } from '../../src/data.js'
...
Given(
  'there are students in the system:',
...
Given('there is a student with:', async function (dataTable: any) {
...
When('I fill in the form with:', async function (dataTable: any) {
  const row = dataTable.rowsHash()
  context.form = {
    name: row.name || '',
    cpf: row.CPF || '',
...
When('I click the edit button for {string}', async function (studentName: string) {
...
When('I update the email to {string}', function (newEmail: string) {
...
When('I click the delete button for {string}', async function (studentName: string) {
  const student = context.students[studentName]
  if (student) {
...
Then('I should see the message {string}', function (message: string) {
  // Verify success response
  expect(context.response.success).to.be.true
  expect(context.statusCode).to.be.oneOf([200, 201])
})

Then('I should see the error {string}', function (errorMessage: string) {
  expect(context.response.success).to.be.false
  expect(context.error).to.equal(errorMessage)
})

Then('the student {string} should appear in the list', async function (studentName: string) {
  const res = await request(app).get('/students')
  const students = res.body.data
  const found = students.some((s: any) => s.name === studentName)
...
Then('I should see {number} students in the list', async function (count: number) {
  const res = await request(app).get('/students')
  const students = res.body.data
  expect(students.length).to.equal(count)
})

Then('the list should contain {string}', async function (studentName: string) {
  const res = await request(app).get('/students')
  const students = res.body.data
  const found = students.some((s: any) => s.name === studentName)
  expect(found).to.be.true
...
Then('the student {string} should have email {string}', async function (studentName: string, email: string) {
  const res = await request(app).get('/students')
  const students = res.body.data
...
Then("the student's name should remain {string}", async function (studentName: string) {
  const res = await request(app).get('/students')
  const students = res.body.data
  const student = students.find((s: any) => s.name === studentName)
  expect(student).to.exist
...
Then('the student {string} should not be in the list', async function (studentName: string) {
  const res = await request(app).get('/students')
  const students = res.body.data
  const found = students.some((s: any) => s.name === studentName)
