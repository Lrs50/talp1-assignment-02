...
import { VALID_GRADES } from './assessmentService.js'
...
export async function getClassDetail(id: string): Promise<ClassDetail | null> {
  ...
  return { cls, students, goals, assessments }
}
...
export async function enrollMultipleStudents(
  classId: string,
  studentIds: string[],
): Promise<{ success: boolean; enrolled: number; skipped: number }> {
  let enrolled = 0
  let skipped = 0
...
async function notifyAssessmentChange(
  studentId: string, classId: string, goal: string, grade: Grade,
): Promise<void> {
  ...
  if (student && cls) {
    await queueNotification({
      studentId, studentName: student.name, studentEmail: student.email,
      classId, className: cls.topic, goal, grade,
    })
  }
}
...
export async function setClassAssessment(
  ...
  if (index !== -1) {
    ...
    await notifyAssessmentChange(studentId, classId, goal, grade)
    return { success: true, data: assessments[index] }
  }
  const assessment: ClassAssessment = {
    id: generateId(), classId, studentId, goal, grade,
